...
class TOTP {
  ...
  static String generateTOTPCodeString(
    ...
    final code = '${generateTOTPCode(secret, time, length: length, interval: interval, algorithm: algorithm, isGoogle: isGoogle)}';
  ...
  static String generateHOTPCodeString(
    ...
    final code = '${generateHOTPCode(secret, counter, length: length, algorithm: algorithm, isGoogle: isGoogle)}';
    return code.padLeft(length, '0');
  }
  ...
  static int remainingSeconds({int interval = 30}) {
    return interval - (((lastUsedTime ~/ 1000)) % interval);
  }

  static String _hexEncode(final Uint8List input) => [
        for (int i = 0; i < input.length; i++) input[i].toRadixString(16).padLeft(2, '0'),
      ].join();
